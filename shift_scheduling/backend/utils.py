...
def save_schedule(*, user: User, res, current_week, db):
    """This function saves a schedule to the database"""
    staff_map = {staff.id: staff for staff in db.exec(select(Staff)).all()}
    department_map = {
        department.id: department for department in db.exec(select(Department)).all()
    }
...
def get_week(week_start: str, db, user: User) -> date:
    week_statement = select(ScheduleWeek).where(
        ScheduleWeek.week_start == week_start, ScheduleWeek.creator == user
    )
    week = db.exec(week_statement).first()

    if not week:
        week_end = week_str_to_object(week_start) + timedelta(days=6)
        week = ScheduleWeek(week_start=week_start, week_end=week_end, creator=user)
        db.add(week)
        db.commit()
        db.refresh(week)
...
def initialize(db, current_week, user: User):
...
def create_schedule(week_start, db, user: User):
    ...
    staff_list, department_list = initialize(db, current_week, user)

    if not staff_list:
        raise ScheduleError("No staff available")

    if not department_list:
        raise ScheduleError("No department available")

    if not current_week.complete:
        res = scheduler(department_list, staff_list)
    else:
        _, _, schedule_res = get_week_schedule(
            user=user, week_start=week_start, db=db, regenerate=True
        )
        ...
        if not updated_res["regenerated"]:
            raise ScheduleError("Existing Schedule not modified", 304)
    ...
    raise ScheduleError("No schedule generated")


def get_week_schedule(*, user: User, db, week_start: str, regenerate: bool = False):
    ...
    schedule_statement = (
        select(Schedule)
        .join(ScheduleWeek)
        .where(ScheduleWeek.creator == user)
