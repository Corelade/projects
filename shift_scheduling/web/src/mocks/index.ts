...
import type { Credentials } from '@/store/api/auth-api'
...
async function route(key: MockKey, body: unknown): Promise<MockResult> {
  ...
  switch (key) {
    ...
    case 'auth.signIn': {
      ...
      // One message for both halves — saying which was wrong turns the sign-in
      // form into a way to test which usernames exist.
      if (!user || user.password !== password) {
        return fail(401, "That username and password don't match.")
      }
      return { data: { token: `mock-token-${user.id}`, user: { id: user.id, username: user.username } } }
    }
    ...
    case 'auth.signUp': {
      ...
      const created = { id: db.nextUserId(), username: name, password }
      db.users.push(created)
      return { data: { token: `mock-token-${created.id}`, user: { id: created.id, username: created.username } } }
    }
